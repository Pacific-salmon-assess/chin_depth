...
dum_test <- test_depth_baked %>% 
  mutate(mean_pred = test_preds$predictions,
         split_group = "test")

all_preds <- rbind(dum %>% select(-resid), dum_test) 

fit_obs <- ggplot() +
  geom_point(
    data = all_preds,
    aes(x = depth, y = mean_pred, fill = split_group),
    shape = 21, alpha = 0.2
    ) +
  labs(
...
png(here::here("figs", "ms_figs", "obs_preds.png"),
...
pdf(here::here("figs", "depth_ml", "counterfactual_ranger_ji.pdf"),
    height = 6, width = 8)
plot_list
mat_plot
dn_plot
...
png(here::here("figs", "ms_figs", "counterfac_effects.png"),
    height = 5, width = 5, 
    units = "in", res = 250)
...
pred_dat2 <- pred_dat1 %>%
  mutate(
    pred_med = pred_rf1$predictions[, "med"],
    pred_lo = pred_rf1$predictions[, "lo"],
    pred_up = pred_rf1$predictions[, "up"],
    utm_x_m = utm_x * 1000,
    utm_y_m = utm_y * 1000,
...
var_depth <- base_plot +
  ...
  geom_sf(data = coast_utm) +
  scale_fill_viridis_c(name = "Prediction Interval Width", 
                       option = "C",
                       direction = -1)  +
  theme(legend.position = "top")
